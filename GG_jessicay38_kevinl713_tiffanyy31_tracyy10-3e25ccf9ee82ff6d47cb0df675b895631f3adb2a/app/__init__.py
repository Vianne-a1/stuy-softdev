...
def leaderboard():
    if 'username' not in session:
        return redirect("/")
    x = setup_db.getTopTen()
    while len(x) != 10:
        x.append(("",))
    y = setup_db.getMessages()
    while len(y) != 3:
        y.append(("This user was too cool for a message",))
    return render_template("leaderboard.html", topTen = x, topThree = y)
...
def settings():
    if 'username' not in session:
        return redirect("/")
    if request.method == "POST":
        setup_db.choose_message(session['username'], request.form.get("bio"))
...
def submitScore():
    if(session.get('username') != None):
        ...
        setup_db.change_high_score(session.get('username'),score)
        setup_db.add_points(session.get('username'),score)
    return '', 204
